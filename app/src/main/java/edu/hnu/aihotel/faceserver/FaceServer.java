package edu.hnu.aihotel.faceserver;
...
import edu.hnu.aihotel.model.FaceRegisterInfo;
...
public class FaceServer {
    ...
    public CompareResult getTopOfFaceLib(FaceFeature faceFeature) {
        ...
        for (int i = 0; i < faceRegisterInfoList.size(); i++) {
            ...
            System.out.println(faceRegisterInfoList.get(i).getName()  + "  的相似度：" + faceSimilar.getScore());
    ...
    * */
    public boolean hasRegister(String userName){
        ...
        if(faceRegisterInfoList!=null){
            for (int i = 0; i < faceRegisterInfoList.size(); i++) {
                if (faceRegisterInfoList.get(i).getName().equals(userName)){
                    result = true;
                    break;
                }
            }
        }
        System.out.println("查找用户"+userName+"是否注册结果： " + result);
        return result;
    }
